...
function fixIdSiteUrl(idSiteUrl){
    // ugly hack to compensate invalid URLs generated by Application.createIdSiteUrl()
    // if cloudpass is not mounted at the root level
    const parsedUrl = url.parse(idSiteUrl);
    parsedUrl.pathname = url.parse(config.get('cloudpass.baseUrl')).pathname.replace(/\/v1$/, parsedUrl.pathname);
    ...
    .get('/:applicationId/login', (req, res) =>
        getApplication(req.params.applicationId)
            .then(application => {
                ...
                res.status(302)
                        .location(fixIdSiteUrl(application.createIdSiteUrl({
                            path: '/#/',
                            state: req.query.sso,
